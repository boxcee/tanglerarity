...
const getNft = async (collectionUid, uid) => {
  const client = await clientPromise;
  const db = client.db(DB_NAME);
  const collection = db.collection(collectionUid);
  return collection.findOne({ _id: uid });
};

const getNfts = async (uid, limit, skip, filter) => {
  const client = await clientPromise;
  const db = client.db(DB_NAME);
  const collection = db.collection(uid);
  const cursor = await collection.find({ collection: uid, ...filter }, { skip }).limit(limit);
...
export { getCollection, createCollection, getNfts, createNfts, getNft };
